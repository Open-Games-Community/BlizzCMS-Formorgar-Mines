      ...
      <div class="uk-container">
        <div class="uk-grid uk-grid-medium" data-uk-grid>
          ...
          <div class="uk-width-3-4@m">
            <h4 class="uk-h4 uk-text-uppercase uk-text-bold" style="color:orange"><?=$this->lang->line('tab_vote');?></h4>
            <div class="uk-grid uk-grid-small uk-child-width-1-1 uk-child-width-2-4@s uk-child-width-2-4@m" data-uk-grid>
              <?php foreach ($voteList as $key => $voteList): ?>
              <div>
                <div class="uk-card uk-card-vote">
                  <div class="uk-card-header">
                    <h5 class="uk-h5 uk-text-uppercase uk-text-bold" style="color:orange"><?=$voteList->name?></h5>
                  </div>
                  <div class="uk-card-body">
                    ...
                    <div class="uk-grid-collapse uk-child-width-auto uk-flex-center uk-margin-small-bottom" uk-grid uk-countdown="date: <?=date('c', $this->vote_model->getTimeLogExpired($voteList->id, $this->session->userdata('wow_sess_id')));?>">
                      <div>
                        <div class="uk-countdown-number uk-countdown-days" style="font-size:25px"></div>
                      </div>
                      <div class="uk-countdown-separator" style="font-size:25px">:</div>
                      <div>
                        <div class="uk-countdown-number uk-countdown-hours" style="font-size:25px"></div>
                      </div>
                      <div class="uk-countdown-separator" style="font-size:25px">:</div>
                      <div>
                        <div class="uk-countdown-number uk-countdown-minutes" style="font-size:25px"></div>
                      </div>
                      <div class="uk-countdown-separator" style="font-size:25px">:</div>
                      <div>
                        <div class="uk-countdown-number uk-countdown-seconds" style="font-size:25px"></div>
                      </div>
                    </div>
                    <?php if ($this->wowgeneral->getTimestamp() >= $this->vote_model->getTimeLogExpired($voteList->id, $this->session->userdata('wow_sess_id'))): ?>
                      <?=form_open(base_url('vote/votenow/' . $voteList->id));?>
                        <button class="uk-button uk-button-default uk-width-1-1" style="color:#1e87f0"><i class="fas fa-vote-yea"></i> <?=$this->lang->line('tab_vote');?></button>
                      <?=form_close();?>
                    <?php else: ?>
                      <?=form_open();?>
